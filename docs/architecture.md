...
Пользователь за один взгляд понимает остаток выбранной квоты, а за один клик — состояние подписок Codex, Claude Code и Cursor. Интерфейс ощущается частью macOS. Изменение источника одного агента не требует менять остальные источники, графики или виджеты.
...
- Малый WidgetKit-виджет: закреплённое окно; средний: обзор трёх агентов. Нажатие открывает приложение.
...
Модульное нативное приложение: Xcode-проект содержит app target и Widget Extension, локальный Swift package содержит независимые модули. SwiftPM сам по себе не заменяет упаковку и подпись extension в Xcode.

Отдельный постоянный daemon пока не нужен: сбором управляет приложение. Это сокращает число процессов, схем авторизации и вариантов восстановления. Один SwiftUI target для всего отвергнут: виджет не должен зависеть от транспорта Codex или авторизации Cursor.
...
flowchart TB
  ...
  Snapshot[Обезличенный WidgetSnapshot]
...
| `AllowanceStorage` | SQLite, 90 дней хранения, экспорт и атомарный снимок виджета | SQL и структура файлов |
...
| `LimitRoomWidgets` | Отображение последнего снимка и deep links | Не получает токены, историю или доступ к API |
...
Подключение bridge — явное действие в настройках LimitRoom. Существующий `statusLine` сохраняется, изменение показывается пользователю и обратимо. LimitRoom не перезаписывает чужую конфигурацию при запуске. До подключения — «Требуется подключение», после него данные зависят от активности Claude; чтение файла каждые пять минут не равно новому запросу к Anthropic.
...
Для сетевых источников — базовый интервал 5 минут, ручное обновление и обновление после сна. У адаптера свой минимальный интервал; concurrent запросы объединяются. Ошибка одного агента не отменяет удачное обновление остальных. Нужны timeout, отмена и backoff; после 429 учитывается Retry-After.
...
`AppModel` остаётся единственным владельцем runtime, снимков и выбора AgentID + WindowID. `PresentationPreferences` хранит режим, показ процента, haptics, содержимое сторон камеры, их ширину и формат сброса; demo не пишет эти значения. `PresentationCoordinator` реагирует на дисплеи, Spaces, сон и presentation options; `ApplicationWindows` переиспользует окна истории и настроек. Отдельный callback изменения компоновки сохраняет раскрытую вкладку и булавку при настройке ширины.
...
Схема истории SQLite и формат WidgetSnapshot не изменены. Боковые блоки занимают место в системном меню: ОС не резервирует для них свободную область. Подробности, ограничения и критерии проверки: [исправления native UI и Cursor](superpowers/specs/2026-09-21-native-indicator-cursor-design.md).
...
В extension передаётся атомарно заменяемый versioned `WidgetSnapshot` через App Group: остатки, подписи, reset, freshness и selection. Токены и полная SQLite-база туда не попадают. WidgetKit сам планирует перерисовку; обещания обновлять виджет каждые пять минут нет.

Основное приложение для direct distribution может быть без App Sandbox; extension sandboxed. App Group и Team ID конфигурируются для подписанной сборки. Без настроенного контейнера extension честно показывает необходимость завершить настройку, не использует произвольный общий путь как обход sandbox.

Пользователь пока без платного Apple Developer. Разработка и сборка возможны; работа установленного виджета/общего контейнера требует отдельной проверки локальной подписи. Developer ID/notarization и распространение считаются отдельным этапом.
...
- Пакет собирается Swift 6; app и widget targets собираются для arm64/x86_64 с deployment target macOS 14.
...
- Локальная сборка, live-интеграция, установленный виджет и подписанный релиз имеют отдельные статусы. Успешная компиляция не доказывает работу всех четырёх.
...
- [Apple local packages](https://developer.apple.com/documentation/xcode/organizing-your-code-with-local-packages), [WidgetKit](https://developer.apple.com/documentation/widgetkit/creating-a-widget-extension), [refresh budget](https://developer.apple.com/documentation/widgetkit/keeping-a-widget-up-to-date).
- [App Groups](https://developer.apple.com/documentation/xcode/configuring-app-groups), [SMAppService](https://developer.apple.com/documentation/servicemanagement/smappservice).
