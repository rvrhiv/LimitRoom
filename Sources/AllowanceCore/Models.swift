...
public struct WidgetReading: Codable, Sendable, Identifiable {
  public var id: String { "\(agent.rawValue)/\(windowID)" }
  public let agent: AgentID
  public let windowID: String
  public let title: String
  ...
  public init(snapshot: AgentSnapshot, window: AllowanceWindow) {
...
public struct WidgetSnapshot: Codable, Sendable {
  ...
  public let pinned: WindowSelection?
  public let readings: [WidgetReading]
  public init(snapshots: [AgentSnapshot], pinned: WindowSelection?) {
    version = 1
    generatedAt = .now
    self.pinned = pinned
    readings = snapshots.flatMap { snapshot in
      snapshot.windows.sorted { ($0.durationMinutes ?? 0) > ($1.durationMinutes ?? 0) }
        .map { WidgetReading(snapshot: snapshot, window: $0) }
    }
  }
}
