...
- Кэш последних значений с пометкой устаревания; чтение источников раз в пять минут и после сна.
...
- Малый и средний WidgetKit-виджеты как отдельный Xcode target без доступа к авторизации и истории.
...
- Sparkle 2.10.0: подписанные обновления, компактная кнопка в панели, проверка подписи перед установкой. Скачивание и перезапуск — по нажатию, не при обнаружении версии.
...
Откройте `LimitRoom.xcodeproj`, выберите схему `LimitRoom`. Для проверки обеих архитектур без подписи:

```sh
xcodebuild -project LimitRoom.xcodeproj -scheme LimitRoom \
  -configuration Release -destination 'generic/platform=macOS' \
  -derivedDataPath build/DerivedData CODE_SIGNING_ALLOWED=NO build
```

Для установки extension нужно настроить реальные подписи app/extension и общий App Group в Xcode. `Config/Local.example.xcconfig` показывает настройки; скопируйте в gitignored `Config/Local.xcconfig`. Чужой Team ID или произвольный общий каталог не используются.

На macOS формат группы привязан к реальной команде подписи. Установку/обмен данными и поведение после сна нужно проверить отдельно. Без этого локальная сборка виджета — доказательство компиляции, не его появления в Notification Center. macOS сама выбирает расписание WidgetKit.
...
- Снимок виджета не содержит email, токенов и полной истории.
...
С 0.4.1 нужно один раз установить 0.5.0 вручную: в старой версии нет updater. В последующих сборках доступна кнопка обновления в нижней части панели и страница «Обновления» в настройках. Проверки можно отключить, установка всегда требует нажатия и перезапускает LimitRoom.
...
`AllowanceCore` → `AllowanceStorage` / `AllowanceConnectors` → `AllowanceRuntime` → macOS app. Виджет зависит только от `AllowanceCore` и разрешённого снимка данных. WebKit и системная авторизация принадлежат слою приложения; декодирование Cursor — адаптеру.
